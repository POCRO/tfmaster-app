...
#### Phase 1.5: 键盘快捷键 [增强] ⏸️

**任务列表：**
- [ ] 实现全局键盘事件监听
- [ ] 支持快捷键：1-4, Space, Enter, N
- [ ] 添加快捷键提示（首次使用时显示）
- [ ] 防止快捷键冲突（输入框聚焦时禁用）
...
**任务列表：**
- [ ] 修改测验页面，从数据库加载每日任务
- [ ] 实现答题结果实时保存到 Supabase
- [ ] 添加 localStorage 备份机制（离线支持）
- [ ] 实现数据冲突解决策略

**验收标准：**
- 答题记录实时保存，刷新页面进度不丢失
- 离线模式下数据保存到本地，恢复网络后自动同步
