...
#### Phase 2.4.1: 邀请码系统 [软上线] ✅

**任务列表：**
- [x] 创建 invite_codes 数据表
- [x] 添加邀请码输入和验证逻辑
- [x] 生成测试邀请码
...
**任务列表：**
- [x] 创建范文数据结构 (`src/types/modelText.ts`)
  ```typescript
  interface ModelText {
    id: string;
    title: string;
  ...
  ```
- [x] 准备 5 篇德福范文（涵盖主要话题）
- [x] 创建范文列表页面 (`app/reading/page.tsx`)
- [x] 创建范文详情页面 (`app/reading/[id]/page.tsx`)
- [x] 实现逐句播放功能
- [x] 实现自动播放功能
- [x] 实现播放控制（播放/暂停/上一句/下一句）
- [x] 添加键盘快捷键（A/D/空格/C）
...
**任务列表：**
- [x] 调整数据结构（去掉level和topic，保留例句字段）
- [x] 创建数据库迁移文件（006_remove_level_and_topic.sql）
- [x] 创建apkg导入脚本（从德语专四.apkg导入2072个词汇）
- [x] 更新TypeScript类型定义和相关代码
- [x] 应用数据库迁移到Supabase
- [x] 执行导入脚本，导入2072个单词
- [x] 创建范文内容管理系统（Markdown格式）
- [ ] 为每个单词添加/优化例句
- [ ] 建立同义词/近义词关联网络

**验收标准：**
- ✅ 词库包含 2072 个词条
- ✅ 每个词条包含完整的语法信息（词性、性别、例句等）
- ✅ 数据结构简化，去除冗余分类字段
- ✅ 范文内容管理系统完成（Markdown → TypeScript）
...
**任务列表：**
- [ ] 优化新单词学习逻辑
  - [ ] 实现"完全学会"机制：需连续两次正确选择才算掌握
  - [ ] 进度条显示完全学会的单词数量（而非答题数量）
  - [ ] 选错后进入单词详细释义页面
  - [ ] 详细释义页面包含：单词、音标、释义、例句、词性等
  - [ ] 从详细页面返回后继续测验流程
- [ ] 添加生词本功能（收藏/标记）
- [ ] 实现错题本（自动收集错误单词）
- [ ] 添加复习模式（只复习错题）
